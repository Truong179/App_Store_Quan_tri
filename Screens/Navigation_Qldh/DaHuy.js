...
const renderDonhang = ({ item }) => {
  return (
      <View style={{ flexDirection: "column", marginTop: 10, borderBottomColor: Colors.grey, borderBottomWidth: 10 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', margin: 10 }}>
              <View style={{ flexDirection: 'row' }}>
                  <Image style={{ width: 30, height: 30, borderRadius: 30, marginRight: 5 }} source={{ uri: item.avatar }} />
                  <Text>{item.name}</Text>
              </View>
              <View>
                  <Text style={{ color: Colors.red }}>{item.trangthai}</Text>

              </View>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', margin: 10 }}>
              <View style={{ flexDirection: 'row' }}>
                  <Image style={{ height: 40, width: 50, marginRight: 5 }} source={{ uri: item.image }} />
